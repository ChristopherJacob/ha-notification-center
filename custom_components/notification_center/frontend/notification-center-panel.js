...
let NotificationCenterPanel = class NotificationCenterPanel extends i {
    ...
    _renderInbox() {
        if (this._notifications.length === 0) {
            return b `<div class="nc-empty-state">
      ...
      <div class="nc-header">
        <h1>Inbox</h1>
        ${this._unreadCount > 0
            ? b `<button class="nc-btn" @click=${this._handleMarkAllRead}>Mark all read</button>`
            : ""}
      </div>
    ...
    _renderRules() {
      ...
      ${this._rules.length === 0
            ? b `<div class="nc-empty-state"><ha-icon icon="mdi:routes"></ha-icon><h3>No rules configured</h3></div>`
    ...
    _renderGroups() {
      ...
      ${this._groups.length === 0
            ? b `<div class="nc-empty-state"><ha-icon icon="mdi:account-group-outline"></ha-icon><h3>No groups configured</h3></div>`
            : b `<div style="display:flex;flex-direction:column;gap:8px;">
            ${this._groups.map((group) => b `
              <div class="nc-card">
                ...
                ${group.targets.length > 0 ? b `<div style="margin-top:8px;display:flex;gap:4px;flex-wrap:wrap;">
                  ${group.targets.map((t) => b `<span style="background:var(--nc-bg-secondary);padding:2px 8px;border-radius:4px;font-size:11px;">${t}</span>`)}
                </div>` : ""}
              </div>`)}
          </div>`}
    `;
